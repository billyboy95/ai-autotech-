...
export default async function IntakePage({ params }: { params: Promise<{ formKey: string }> }) {
  ...
  const name = org?.name ?? (formKey === "eastc" ? "EASTC" : formKey === "ai-autotech" ? "AI AutoTech" : "this workspace");
  const color = org?.primaryColor ?? (formKey === "eastc" ? "#0F3D4C" : "#0B1F3A");
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#F3F4F6] px-4 py-10">
      <section className="w-full max-w-md rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {org || formKey === "eastc" || formKey === "ai-autotech" ? (
